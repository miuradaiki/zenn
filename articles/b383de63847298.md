...
今回はデータベースやスキーマは分けず、
テナントIDによる行の特定を行うプール型を採用します。

この辺りの分離モデルについては後ほどご説明します。

### マルチテナント系のgemについて

今回RLSを使用するにあたってマルチテナント系のgemは使いません。

ブリッジ型であれば「[apartment](https://github.com/influitive/apartment)」
プール型であれば「[activerecord-multi-tenant](https://github.com/citusdata/activerecord-multi-tenant)」と「[activerecord-tenant-level-security](https://github.com/kufu/activerecord-tenant-level-security)」
